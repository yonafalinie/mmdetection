...
class OLNKMeansFFSRoIHead(OLNKMeansVOSRoIHead):
    ...
    def focal_loss(self, inputs, targets, alpha=1, gamma=-0.75):
        """
        Compute focal loss for binary classification
        inputs: Tensor of predictions (logits)
        targets: Tensor of true labels (0 or 1)
        alpha: Weighting factor for the positive class (default: 0.25)
        gamma: Focusing parameter to down-weight easy examples (default: -0.75 to match given equation)
        """
        # Convert logits to probabilities
        pt = torch.sigmoid(inputs)  # Probability of the correct class
        # print('pt', pt)
        
        # Compute binary cross-entropy
        BCE_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction='none')

        # Apply the focal loss formula with gamma = -0.75
    ...
    def _ood_forward_train(self, bbox_results, bbox_targets, bbox_weak_results, bbox_weak_targets, device):
        ...
        if self.kmeans is not None:
            ...
            if self.epoch >= self.start_epoch:
                ...
                ood_reg_loss = self.focal_loss(output.view(-1), labels_for_loss, gamma=-0.75)
