...
class CLIPViTAdapter(nn.Module):
    def __init__(self, arch, patch_size, drop_path_rate, out_indices, init_cfg=None):
        ...
        self.vit = VisionTransformer(
            ...
            with_cls_token=True,  # CLIP includes CLS token
            out_type='raw',  # Full patch sequence
            final_norm=False,
            init_cfg=init_cfg)
    ...
    def forward(self, x):
        ...
        batch_size = x.size(0)
        h, w = x.size(2) // self.patch_size, x.size(3) // self.patch_size  # e.g., 41, 25
        ...
        for feat in feats:
            ...
            if feat.dim() == 3:
                ...
                feat = feat.view(batch_size, h, w, -1).permute(0, 3, 1, 2)  # [2, 768, 41, 25]
                out.append(feat)
            else:
