...
from projects.OLN.oln import ConvFCBBoxScoreHead
from projects.OLN.oln.convfc_bbox_score_head import Shared2FCBBoxScoreHead

import torch.nn.functional as F


def multiclass_nms_with_ood(
    multi_bboxes: Tensor,
    multi_scores: Tensor,
    multi_ood_scores: Tensor,
    score_thr: float,
    nms_cfg: ConfigType,
    max_num: int = -1,
    ...
    if bboxes.numel() == 0:
        ...
        if return_inds:
            return dets, labels, multi_ood_scores, inds
        else:
            return dets, labels, multi_ood_scores
    id_indices = multi_ood_scores >= ood_score_threshold
    ood_indices = multi_ood_scores < ood_score_threshold
    _labels = labels * id_indices.to(torch.int8) + \
              (labels.max() + 1) * (1 - id_indices.to(torch.int8))
    dets, keep = batched_nms(bboxes, scores, _labels, nms_cfg)
    ...
    if return_inds:
        return dets, labels[keep], multi_ood_scores[keep], inds[keep]
    else:
        return dets, labels[keep], multi_ood_scores[keep]


@MODELS.register_module()
class OODShared2FCBBoxScoreHead(Shared2FCBBoxScoreHead):
    ...
    def predict_by_feat(self,
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            oods = F.sigmoid(ood_scores[img_id][:, 0])
    ...
    def _predict_by_feat_single(self,
        ...
        if rcnn_test_cfg is None:
            ...
            results.ood_scores = ood_score
        else:
            det_bboxes, det_labels, det_ood_scores = multiclass_nms_with_ood(
                bboxes,
                scores,
                ood_score,
                ...
                box_dim=box_dim,
                ood_score_threshold=rcnn_test_cfg.ood_threshold)
            results.bboxes = det_bboxes[:, :-1]
            results.scores = det_bboxes[:, -1]
            results.labels = det_labels
            results.ood_scores = det_ood_scores
        return results
